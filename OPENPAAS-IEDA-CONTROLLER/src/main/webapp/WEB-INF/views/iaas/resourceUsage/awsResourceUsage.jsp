...
$(function() {
    ...
    setAwsRegion = setDefaultAwsRegion();
    doSearch();
});
...
function doSearch(){
    ...
    getIAwsResourceUsageInfo();
}


/****************************************************
 * 기능 : getIAwsResourceUsageInfo
 * 설명 : AWS 리소스 조회 요청
*****************************************************/
function getIAwsResourceUsageInfo(){
     w2utils.lock($("#layout_layout_panel_main"), search_lock_msg, true);
    $.ajax({
        type : "GET",
        url : "/iaasMgnt/resourceUsage/aws/list/"+region,
        ...
        error : function(request, status, error) {
            ...
            $(".panel-body .instanceUsage").html("<p>AWS 인스턴스 사용량 정보가 존재하지 않습니다.</p>");
            $(".panel-body .networkUsage").html("<p>AWS 네트워크 사용량 정보가 존재하지 않습니다.</p>");
            $(".panel-body .volumeUsage").html("<p>AWS 볼륨 사용량 정보가 존재하지 않습니다.</p>");
