...
public class BootstrapDeleteDeployAsyncService{
	...
	*****************************************************************/
	public void deleteBootstrapDeploy(BootStrapDeployDTO.Delete dto, Principal principal) {
		...
		try {
			String deloyStateFile = LocalDirectoryConfiguration.getDeploymentDir() + System.getProperty("file.separator") +vo.getDeploymentFile().split(".yml")[0] + "-state.json";
			...
			if ( !stateFile.exists() ) {
				...
				String deployFile = LocalDirectoryConfiguration.getDeploymentDir() + System.getProperty("file.separator") + vo.getDeploymentFile();
