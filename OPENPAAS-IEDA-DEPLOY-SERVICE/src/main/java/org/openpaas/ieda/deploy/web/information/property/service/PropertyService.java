package org.openpaas.ieda.deploy.web.information.property.service;
...
public class PropertyService {
    ...
    ***************************************************/
     ...
     * @return : PropertyVO
    ***************************************************/
    public PropertyVO getPropertyDetailInfo(String deployment, String name) {
        DirectorConfigVO defaultDirector = directorConfigService.getDefaultDirector();
        //2. 설치가 없을 경우
        if ( defaultDirector == null ) {
            throw new CommonException("notfound.director.exception",
        ...
        try {
            HttpClient httpClient = DirectorRestHelper.getHttpClient(defaultDirector.getDirectorPort());
            get = new GetMethod(DirectorRestHelper.getPropertyDetailInfoURI(defaultDirector.getDirectorUrl(), defaultDirector.getDirectorPort(),deployment, name));
            get = (GetMethod)DirectorRestHelper.setAuthorization(defaultDirector.getUserId(), defaultDirector.getUserPassword(), (HttpMethodBase)get);
            httpClient.executeMethod(get);
            if ( !StringUtils.isEmpty(get.getResponseBodyAsString()) ) {
                ObjectMapper mapper = new ObjectMapper();
                vo = mapper.readValue(get.getResponseBodyAsString(), PropertyVO.class);
                vo.setName(name);
                vo.setDeploymentName(deployment);
            }
        } catch(RuntimeException e){
            throw new CommonException("runtime.property.excepion",
     ...
     * @return : void
    ***************************************************/
    public void createProperyInfo(PropertyDTO dto) {
     ...
     * @return : void
    ***************************************************/
    public void updateProperyInfo(PropertyDTO dto) {
        DirectorConfigVO defaultDirector = directorConfigService.getDefaultDirector();
        int statusCode;
        if ( defaultDirector == null ) {
            throw new CommonException("notfound.director.exception",
     ...
     * @return : void
    ***************************************************/
    public void deleteProperyInfo(PropertyDTO dto) {
        DirectorConfigVO defaultDirector = directorConfigService.getDefaultDirector();
        int statusCode;
        if ( defaultDirector == null ) {
            throw new CommonException("notfound.director.exception",
