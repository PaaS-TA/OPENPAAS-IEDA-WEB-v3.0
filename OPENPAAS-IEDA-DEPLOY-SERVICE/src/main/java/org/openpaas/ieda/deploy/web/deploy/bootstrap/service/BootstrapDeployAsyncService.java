	...
	*****************************************************************/
	public void deployBootstrap(BootStrapDeployDTO.Install dto, Principal principal) {
		...
		try {
			...
			String deployFile = LocalDirectoryConfiguration.getDeploymentDir() + System.getProperty("file.separator") + bootstrapInfo.getDeploymentFile();
			File deploymentFile = new File(deployFile);
			
			if( deploymentFile.exists() ) {
				//1. 배포상태 설정
			    bootstrapInfo.setUpdateUserId(principal.getName());
				bootstrapInfo.setDeployStatus( message.getMessage("common.deploy.status.processing", null, Locale.KOREA) );
			...
			if ( status.equals("error") ) {
				...
				if ( accumulatedLog.contains("Failed deploying") || accumulatedLog.contains("Failed")) {
					...
					bootstrapInfo.setDeployStatus(message.getMessage("common.deploy.status.failed", null, Locale.KOREA) );
					saveDeployStatus(bootstrapInfo);
					DirectorRestHelper.sendTaskOutput(principal.getName(), messagingTemplate, MESSAGE_ENDPOINT, "error", Arrays.asList("", "BOOTSTRAP 설치 중 오류가 발생하였습니다."));
				}	else {
					// 타겟 테스트
			...
			if(lockFile.exists()){
				...
				if( LOGGER.isDebugEnabled() ) LOGGER.debug("check delete lock File  : "  + check); 
			}
		}
	}
