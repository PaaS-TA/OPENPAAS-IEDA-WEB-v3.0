    ...
    public List<VmsListDTO> getVmList(String deployment){
        ...
        try{
            ...
            if (HttpStatus.valueOf(vmStatusCode) != HttpStatus.OK) {
                throw new CommonException("notfound.vm.exception",  " VM 정보가 존재 하지 않습니다.", HttpStatus.BAD_REQUEST);
            }
            ...
            if (taskStatusCode == HttpStatus.OK.value() || taskStatusCode == HttpStatus.PARTIAL_CONTENT.value()) {
                ...
                while ((info = bufferedReader.readLine()) != null){ 
                    ...
                    if( "null".equalsIgnoreCase(obj.get("az").toString()) &&  !(StringUtils.isEmpty(obj.get("az").toString())) ){
                    ...
                    for(int i=0; i < obj.getJSONArray("ips").length(); i ++){
                        ips.append(obj.getJSONArray("ips").get(i).toString()).append("<br/>");
                    }
                    ...
                    while( iterator.hasNext() ){
                        if( "persistent".equalsIgnoreCase(iterator.next().toString()) ){
                    ...
                    for(int i=0; i < vitalsObj.getJSONArray("load").length(); i ++){
                        load.append(vitalsObj.getJSONArray("load").get(i).toString()).append("<br/>");
                    }
            ...
            throw new CommonException("runtime.vm.exception", "VM 정보를 가져올 수 없습니다. ", HttpStatus.INTERNAL_SERVER_ERROR);
        }catch(Exception e){
